...
| Угроза | Результат |
|---|---|
| Компрометация ключа оператора | Нельзя подделать backed refill (верификатор отклонит фиктивное событие). Нельзя обойти HTLC claim/refund (settlement через preimage). Нельзя тронуть USD-stable, уже залоченные в HTLC-свопах. **Можно** перевести тот USD-stable баланс, который лежит на аккаунте оператора в момент компрометации. Аккаунт оператора — обычный аккаунт; его баланс — это float между `refill_mint` (кредитует) и `HTLC.lock` (дебитует). Чем быстрее оператор пускает refill в lock, тем меньше float. Скомпрометированный ключ также может вызвать `refill_mint` против реальных ожидающих Ethereum-локов и сразу увести средства — пользователи сделают refund на Ethereum, но на стороне 2D supply станет необеспеченным. Exposure = текущий баланс оператора + mintable-but-unminted Ethereum-локи. HTLC убирает класс `unlock()`-атак, но не убирает custodial risk на hot-wallet балансе оператора. |
...
Мост убирает `unlock()`-полномочия, которые позволяли сливать wrapped-мосты, и привязывает каждый минт к Ethereum-событию, проверенному верификатором. Это **не** убирает custodial risk на аккаунте оператора. Ключ оператора — ключ hot wallet; компрометация открывает доступ к текущему балансу (float между `refill_mint` и `HTLC.lock`) плюс к реальным Ethereum-локам, которые ещё не были обработаны. Средства, уже залоченные в HTLC-свопах, защищены: их может вывести только `claim(preimage)` или `refund(hash)`.
