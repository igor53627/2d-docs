...
| Угроза | Результат |
|---|---|
| Компрометация ключа оператора | Нельзя подделать backed refill (верификатор отклонит фиктивное событие). Нельзя обойти HTLC claim/refund (settlement через preimage). Нельзя тронуть USD-stable, уже залоченные в HTLC-свопах. Нельзя делать обычные переводы: block executor блокирует non-precompile переводы с адреса оператора, поэтому средства можно двигать только через precompile-вызовы. **Можно** вызвать `refill_mint` против реальных ожидающих Ethereum-локов и сразу залочить через `bridge_lock`; при `bridge_lock` средства идут напрямую в HTLC escrow для указанного получателя, не оставляя свободного float. Exposure ограничен mintable-but-unminted Ethereum-локами. HTLC убирает класс `unlock()`-атак; ограничение переводов оператора дополнительно снижает custodial risk, блокируя прямое извлечение баланса. |
...
Мост убирает `unlock()`-полномочия, которые позволяли сливать wrapped-мосты, и привязывает каждый минт к Ethereum-событию, проверенному верификатором. Ограничение переводов оператора блокирует обычные переводы с адреса оператора, поэтому даже скомпрометированный ключ не может извлечь средства иначе как через precompile-вызовы. При `bridge_lock` refill и HTLC lock происходят атомарно: нет окна, когда замinted средства лежат как свободный баланс оператора. Exposure ограничен Ethereum-локами, которые уже finalized, но ещё не обработаны. Средства, уже залоченные в HTLC-свопах, защищены: их может вывести только `claim(preimage)` или `refund(hash)`.
