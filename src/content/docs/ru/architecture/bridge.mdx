...
| Угроза | Результат |
|---|---|
| Компрометация ключа оператора | Нельзя подделать backed refill (верификатор отклонит фиктивное событие). Нельзя обойти HTLC claim/refund (settlement через preimage). **Можно** перевести USD-stable с operator account, который является обычным аккаунтом с переводимым балансом. Пул оператора — по сути hot wallet: компрометация ключа позволяет увести всю накопленную ликвидность, оставив минтированный supply необеспеченным. HTLC убирает класс `unlock()`-атак, но не убирает custodial risk на самом аккаунте оператора. |
...
Мост убирает `unlock()`-полномочия, которые позволяли сливать wrapped-мосты, и привязывает каждый минт к Ethereum-событию, проверенному верификатором. Это **не** убирает custodial risk на аккаунте оператора: ключ оператора — ключ от hot wallet, и его компрометация открывает доступ к тому USD-stable балансу, который оператор держит в пуле в данный момент.
