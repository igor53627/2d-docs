...
- **Host A: оркестратор.** Elixir-процесс моста: HTTP API, watcher сети Ethereum, база оператора. Это самый открытый наружу компонент, через него идёт пользовательский трафик. Считается наиболее подверженным компрометации, даже несмотря на то, что это наш собственный код.
