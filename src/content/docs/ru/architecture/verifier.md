...
description: Как развернуть верификатор, который независимо воспроизводит каждый блок, отдаёт проверенное состояние кошелькам и отклоняет всё, что производитель блоков вычислил неправильно.
---

Верификатор это read-only нода, которая воспроизводит каждый блок от производителя на своей копии состояния. Если state root или block hash не совпадают, блок отклоняется, нода перестаёт обслуживать запросы. Пользователи и кошельки подключаются к RPC верификатора, а не напрямую к producer.
...
Верификатору нужны два изменения в конфиге по сравнению с производителем:
...
Верификатору нужна своя база данных. Общего хранилища с производителем нет.
...
| Проверка | Что ловит |
|----------|-----------|
| **state_root** | Производитель записал состояние, не следующее из транзакций. Охватывает балансы, HTLC swaps, регистрации precompile. |
...
| **Восстановление отправителя** | Для Ethereum-транзакций sender заново выводится из подписи. Для Tron-транзакций подпись перепроверяется против заявленного sender. |
...
| **Bridge cross-chain check** | Каждая строка `bridge_mints` независимо проверяется по финализированному Ethereum-стейту через JSON-RPC. Для строк `bridge_lock` адрес `receiverOn2D` из Ethereum-события `Locked` сравнивается с получателем HTLC на 2D. Полная таблица проверок в статье [Мост](../bridge/). |
...
Каждый верификатор ретранслирует проверенные блоки на своём block feed. Второй верификатор может подписаться на него вместо producer-а:
...
У producer-а нет публичных HTTP-портов. Весь пользовательский трафик идёт через верификатор.
