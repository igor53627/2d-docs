...
Каждый хеш строки (row hash) кодирует поля в каноническом бинарном формате (целые числа фиксированной ширины, строки с префиксом длины, нормализованные значения Decimal). Два арифметически равных баланса всегда дают одинаковую последовательность байт, независимо от внутреннего представления объекта Decimal.
...
**Транзакции (от пользователей к производителю):** пользователи отправляют транзакции на RPC верификатора (это единственный публичный endpoint). Верификатор пересылает каждую транзакцию производителю асинхронным сообщением (fire-and-forget) по Erlang distribution. Производитель записывает ее в mempool и обрабатывает при формировании следующего блока.
