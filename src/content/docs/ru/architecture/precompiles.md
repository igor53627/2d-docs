...
Namespace адреса тоже имеет значение. Все системные precompile-ы живут по `0x2D00…`; верхний байт `0x2D` (ASCII `-`, от названия проекта) это зарезервированный префикс. `eth_getCode` возвращает `0x01` для любого адреса в этом диапазоне, который резолвится, так что кошельки, которые ветвятся по «есть ли тут код?», получают правильный ответ без необходимости иметь полную EVM.

## Скетч HTLC precompile

Говорить о precompile-ах в абстракте можно только до какого-то момента. Ниже — целевой дизайн HTLC precompile, первого precompile с реальным состоянием в нашей дорожной карте. Код ещё не написан, только форма; этого хватит, чтобы увидеть, как precompile выглядит в жизни.
...
defmodule Chain.Precompiles.HTLC do
  ...
  Целевой дизайн первого настоящего precompile-а.
  """
  @behaviour Chain.Precompile
...
- **Меньше экосистема.** Весь dApp-стек в EVM-совместимых сетях предполагает Solidity + EVM. Precompile-цепь это другой target: интеграции требуют обхода ABI каждого precompile-а, а не натравливания существующего Solidity-тулчейна.
...
- **Ноль VM-attack-surface.** Нет EVM-багов, нет байт-код-эксплойтов, нет тонких оракулов opcode-интеракций. Attack-surface это несколько precompile-модулей плюс сам BEAM. Существенное сокращение.
...
Каждый ярус дороже и сильнее предыдущего. Отдельная статья по верификации выйдет, когда появится настоящий precompile, о котором есть что доказывать. Абстрактные доказательства ни о чём никому не нужны.
...
Следующая статья серии выйдет уже с кодом на руках и полным разбором пути доказательств: от типов до формальной спеки протокола. Писать её раньше смысла мало.
