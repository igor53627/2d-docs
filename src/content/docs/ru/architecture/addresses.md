...
Когда вы первый раз заходите в 2D из TronLink и дёргаете `/wallet/getaccount`, в ответе приходит строка вида `T…`. Запросите тот же баланс через MetaMask (`eth_getBalance`) — и получите `0x…`. Это **один и тот же аккаунт**: не копия, не зеркало, а две формы записи одного 20-байтного ключа, который сеть хранит в единственном экземпляре.

В статье разбираемся, почему оба представления существуют, что у них общего, что их различает, и где в `lib/chain/` они сходятся.
...
Поверхность `/wallet/*` в 2D принимает **четыре формы адреса on-wire**, и каждая разрешается в один и тот же 20-байтный ключ:
