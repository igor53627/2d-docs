...
На деле это не так. Под обеими строками скрывается **один и тот же 20-байтный аккаунт**, полученный одинаковым образом из открытого ключа secp256k1. Всё, что их различает: префикс `0x` или `T`, чередование заглавных и строчных букв, а также алфавит Base58. Все эти отличия существуют лишь на уровне кодирования поверх базовых 20 байт.

2D — это сеть, которая хранит аккаунты в едином 20-байтном формате и предоставляет их наружу в любом из двух представлений. Перевод, отправленный из TronLink, поступает на тот же аккаунт, который MetaMask затем видит через вызов `eth_getBalance`. В этой статье мы последовательно разберем: как формируется сам 20-байтный адрес, две формы его записи (EIP-55 hex и Base58Check), и где именно в `lib/chain/` сеть определяет, какой из форматов был передан на вход.
...
В этом формате больше ничего нет: ни байта версии, ни отдельных байт контрольной суммы. Проверка сводится к следующему: длина строки должна составлять 42 символа, регистр должен быть смешанным, а чередование заглавных и строчных букв должно совпадать с результатом `keccak256(lowercase)`. См. реализацию в [`Chain.Crypto.encode_address/1`](https://github.com/igor53627/2d/blob/4d955b70efde1075e316d9ab2c2c10820fb0cd71/lib/chain/crypto.ex#L380-L397):
...
HTTP API `/wallet/*` в сети 2D принимает **четыре формата адресов на сетевом уровне**, и все они преобразуются в один и тот же 20-байтный ключ:
...
| 21-байтный hex с `0x` | `0x41…` (42 символа) | удаление `0x` → декодирование hex → проверка первого байта `0x41` → извлечение последних 20 байт |
| Сырой 21-байтный hex | `41…` (42 символа) | декодирование hex → проверка первого байта `0x41` → извлечение последних 20 байт |
| Ethereum 20-байтный hex | `0xf39Fd6…` (42 символа) | удаление `0x` → декодирование hex → 20 байт |
