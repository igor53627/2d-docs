...
description: Как 2D отдаёт safety-state моста через Prometheus и какие четыре слоя observability оператор скрейпит, чтобы ловить отказ canary, отклонения верифайера и потерю liveness watchdog'а.
---
