...
5. **Single-use claimer hash.** An `eth_call` to `claimerUsedHash(claimer, htlc_hash)` on the bridge contract must return zero, queried at the finalized head as of verification time (not at the receipt block, since `claimerUsedHash` is set only by `claim()` and is therefore always zero at the block where the `Locked` event was emitted). This catches a separate exploit class from the claimer-allowlist binding: an attacker with the 2D-side key citing a *legacy* `Locked` event whose `claimer` is the legitimate (allowlisted) operator and whose preimage has already been revealed by a prior Ethereum-side claim. Check 3 trivially passes there (the legitimate operator IS in the allowlist); only the finalized-head query of `claimerUsedHash` rejects. The two bindings cover non-overlapping attacker capabilities and are both load-bearing.
...
The bridge operator is a single party operationally but uses two cryptographically distinct keys: a **2D-side key** that signs `bridge_lock` precompile calls (the block executor blocks every other transaction type from this address), and an **Ethereum-side key** that signs `claim(...)` on `BridgeHTLC.sol` and custodies the accumulated USDC pool. The two keys are held by separate signers, and each signer runs a vendor-side allowlist policy: the 2D-side signer refuses anything except the `bridge_lock(...)` selector to the bridge precompile; the Ethereum-side signer refuses anything except `claim(address,bytes32,bytes32)` to the configured `BridgeHTLC` contract. ERC-20 transfers from the operator pool, `lock()`, `refund()`, and calls to any other contract are rejected at the policy layer before the HSM is asked to sign. Compromise of each key is treated as an independent event below.
...
| Joint compromise of both operator keys | With the Ethereum-side key the attacker calls `BridgeHTLC.lock()` from the legitimate operator address (which IS in the allowlist), pinning `claimer = operator` and `receiverOn2D = attacker`; the cited event passes the claimer-allowlist binding trivially, and the 2D-side key then mints unbacked 2D supply for the attacker. After the attacker claims on 2D, they use the Ethereum-side key to claim the original USDC back to the operator pool — net effect is fresh unbacked 2D supply, recyclable across hashes up to the operator pool ceiling. The Ethereum-side key additionally enables direct ERC-20 drain of the pool. The signing-service allowlist policy is the load-bearing operational mitigation here: the Ethereum-side signer refuses to sign `BridgeHTLC.lock()` (the round-trip step) and any ERC-20 `transfer` from the pool (the drain step) before the HSM is reached; the 2D-side signer refuses anything except `bridge_lock(...)` to the bridge precompile. Both rules must be configured at the vendor (Fireblocks/Anchorage/Safe with policy module) or in a self-hosted signer adapter — keys held in a vanilla EOA without policy enforcement do not provide this defense. The protocol-level follow-up is an `OperatorVault` smart-contract wallet that refuses to call `lock()` from the operator wallet on-chain; both layers eventually coexist as defense-in-depth. |
