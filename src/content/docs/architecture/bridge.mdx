...
| Threat | Outcome |
|---|---|
| Operator key compromise | Cannot forge a backed refill (verifier catches fabricated events). Cannot bypass HTLC claim/refund (preimage-locked). **Can** move USD-stable out of the operator pool, which is a regular account with a transferable balance. This makes the pool a hot wallet: key compromise lets the attacker drain accumulated liquidity, leaving minted supply unbacked. HTLC eliminates the `unlock()` class of attacks but does not eliminate custodial risk on the operator account itself. |
...
The bridge eliminates the `unlock()` authority that has drained wrapped bridges, and binds every mint to a verifier-checked Ethereum event. It does **not** eliminate custodial risk on the operator's pool account: the operator key is a hot wallet key, and key compromise exposes whatever USD-stable balance the operator holds at that moment.
