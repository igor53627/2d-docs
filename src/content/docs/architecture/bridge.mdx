...
| Joint compromise of both operator keys | With the Ethereum-side key the attacker calls `BridgeHTLC.lock()` from the legitimate operator address (which IS in the allowlist), pinning `claimer = operator` and `receiverOn2D = attacker`; the cited event passes the claimer-allowlist binding trivially, and the 2D-side key then mints unbacked 2D supply for the attacker. After the attacker claims on 2D, they use the Ethereum-side key to claim the original USDC back to the operator pool — net effect is fresh unbacked 2D supply, recyclable across hashes up to the operator pool ceiling. The Ethereum-side key additionally enables direct ERC-20 drain of the pool. Mitigations: an EIP-712 admission policy on the orchestrator (the operator processes only `Locked` events whose tuple matches a user-pre-registered intent — but bypassed when both keys are compromised); the Ethereum signing-service allowlist (refuses non-`claim` signing from the operator key); the in-design `OperatorVault` (on-chain refusal to call `lock()` from the operator wallet — the durable answer). |
