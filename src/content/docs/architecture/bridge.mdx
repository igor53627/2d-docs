...
This is where the verifier earns its keep. After the producer executes a candidate block, but before the verifier accepts it, every new `bridge_mints` row gets an independent cross-chain check. Each row drives three Ethereum JSON-RPC roundtrips:

- `eth_getTransactionReceipt(tx_hash)` returns the receipt; the log at `log_index` is inspected. The `receiverOn2D` address and `amount` are extracted from `log.data` (non-indexed fields); the `htlc_hash` and `sender` come from indexed topics.
...
After confirming the Locked event exists and is finalized, the verifier performs three additional binding checks:
...
2. **Receiver binding.** The `receiverOn2D` address from the Ethereum event's `topics[3]` must match the receiver in `htlc_swaps`. This prevents routing funds to an attacker address while citing a legitimate lock.
...
`verify_locked_event` returns a map containing `receiver_on_2d` (from `topics[3]`), `htlc_hash_on_eth` (from `topics[1]`), `sender_on_eth` (from `topics[2]`), and `receipt_block`. The cross-chain check uses these to verify all three bindings. `check_not_refunded` queries for `Refunded` events scoped to the specific sender and hash within a bounded block window.
...
3. **Operator locks on 2D (atomic).** Operator submits `bridge_lock(chain_id, tx_hash, log_index, amount, hash, Alice, deadline)` to `0x2D00…0003`. The precompile atomically inserts an HTLC swap (locking `amount` for Alice under `hash`) and a `bridge_mints` row (supply backing). Operator balance stays at zero throughout. The verifier independently re-checks the Ethereum event and verifies that `htlc_hash`, `receiverOn2D`, and refund status all match; on success the block is committed, on failure the block is rejected.
...
| Threat | Outcome |
|---|---|
| Operator key compromise | Cannot forge a backed mint (verifier catches fabricated events). Cannot bypass HTLC claim/refund (preimage-locked). Cannot substitute an attacker-controlled HTLC hash (verifier binds `htlc_hash` to Ethereum `topics[1]`). Cannot route funds to an attacker address (verifier binds receiver to `topics[3]`). Cannot mint against a refunded lock (verifier checks for `Refunded` events). Cannot make plain transfers (block executor blocks non-precompile transfers from the operator address). Operator balance is always zero: `bridge_lock` atomically mints and locks, so there is no unbound-float window. Exposure is limited to the HTLC `lock`/`claim` paths on existing swaps. |
...
The bridge eliminates the `unlock()` authority that has drained wrapped bridges. Every mint is atomically bound to a receiver-specific HTLC via `bridge_lock`, so the operator never holds unbound balance. The verifier independently confirms three bindings (HTLC hash, receiver, refund status) against finalized Ethereum state before accepting any block containing a mint. Funds locked in HTLC swaps are safe: only `claim(preimage)` or `refund(hash)` can move them.
