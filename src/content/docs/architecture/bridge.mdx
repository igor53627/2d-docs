...
| Threat | Outcome |
|---|---|
| Operator key compromise | Cannot forge a backed refill (verifier catches fabricated events). Cannot bypass HTLC claim/refund (preimage-locked). Cannot touch USD-stable already locked in HTLC swaps. **Can** transfer whatever USD-stable balance sits on the operator account at the moment of compromise. The operator account is a regular account; its balance is the float between `refill_mint` (credits) and `HTLC.lock` (debits). The faster the operator deploys refills into HTLC locks, the smaller this float. A compromised key can also `refill_mint` against real pending Ethereum locks and immediately move the proceeds, leaving that portion of supply unbacked (users refund on Ethereum, but the minted tokens are gone). Exposure = current operator balance + mintable-but-unminted Ethereum locks. HTLC eliminates the `unlock()` class of attacks but does not eliminate custodial risk on the operator's hot-wallet balance. |
...
The bridge eliminates the `unlock()` authority that has drained wrapped bridges, and binds every mint to a verifier-checked Ethereum event. It does **not** eliminate custodial risk on the operator account. The operator key is a hot-wallet key; compromise exposes the current account balance (the float between `refill_mint` and `HTLC.lock`) plus any real Ethereum locks that haven't been processed yet. Funds already locked in HTLC swaps are safe — only `claim(preimage)` or `refund(hash)` can move them.
