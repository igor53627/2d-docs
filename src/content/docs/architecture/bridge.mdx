...
The unlock authority is gone. There is no `unlock()` callable by the operator. Funds can only move via `claim(preimage)` — which works only if `sha256(preimage) = hash` and only before the deadline — or via `refund(hash)`, which returns funds to the original sender after the deadline passes with no claim. Preimages originate in user wallets: the user picks the preimage, publishes `hash = sha256(preimage)`, and reveals the preimage only at claim time. A compromised **2D-side** operator key alone cannot forge a 2D HTLC for an attacker: `bridge_lock` only mints when the verifier finds a matching `Locked` event on Ethereum (same hash, same `receiverOn2D`, not yet refunded, not yet consumed by the claimer). Drain paths that require the **Ethereum-side** operator key, including the joint-compromise case, are analyzed in the [trust model](#trust-model-summary) below.
...
| Threat | Outcome |
|---|---|
| 2D-side operator key compromise | The 2D key has no Ethereum-side capability. `bridge_lock` requires a verifier-confirmed Ethereum `Locked` event matching the cited hash, `receiverOn2D`, finality, refund status, and an unused claimer hash — none of which the 2D key can produce on its own. The 2D key is also blocked by the block executor from making plain transfers, so it has no extraction path on 2D either. Operator balance on 2D is always zero: `bridge_lock` atomically mints and locks. Worst case is DoS: `bridge_lock` calls are refused or delayed, in-flight 2D HTLCs `refund` after their deadlines. |
| Ethereum-side operator key compromise | The Ethereum key is a normal EOA holding the USDC pool accumulated from completed bridge-ins. Without policy enforcement at the signer, an attacker can transfer that pool out — bridge-out exits stop until the pool is refilled. User-locked USDC in pending bridge-ins is unaffected: those locks have user-chosen preimages that the attacker does not have. The 2D supply is unaffected: minting requires the 2D-side key. Operational mitigation: scope the Ethereum signer to `claim(address,bytes32,bytes32)`-only on the configured `BridgeHTLC` contract via signing-service policy (Fireblocks/Anchorage/Safe with policy module/self-hosted signer in front of HSM). Protocol-level mitigation in design: replace the EOA with an `OperatorVault` smart-contract wallet that enforces the same allowlist on-chain. |
| Joint compromise of both operator keys | An attacker self-funds an Ethereum `BridgeHTLC.lock()` under attacker-chosen `claimer` and `receiverOn2D`, cites that event via `bridge_lock` (all four 2D verifier bindings pass on a self-consistent attacker-controlled lock), claims the minted 2D supply via the attacker-known preimage, then `claim`s or `refund`s the original USDC back on Ethereum. Net effect: unbacked 2D supply up to the Ethereum-side operator balance. Mitigations are layered: an EIP-712 admission policy on the orchestrator (the operator only processes `Locked` events whose tuple matches a user-pre-registered intent); the Ethereum signing-service allowlist (the signer refuses to sign `BridgeHTLC.lock()` from the operator key); and the in-design `OperatorVault` (protocol-level allowlist on-chain). The first two are operational controls; the third moves the policy on-chain. |
