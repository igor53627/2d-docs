...
5. **Claimer/hash storage probe.** The verifier calls `claimerUsedHash(claimer, htlc_hash)` at `eth_observation_block` persisted on the `bridge_mints` row (consensus state, included in the state root). The operator embeds the current Ethereum `"finalized"` block number when signing `bridge_lock`; replay uses that frozen value, so a later on-chain claim does not change what historical verifiers read. Querying `receipt_block` is vacuous (the slot is always zero there); querying the live finalized head breaks replay after a legitimate claim. This binding complements the claimer allowlist: it catches already-claimed locks even when the cited `claimer` is allowlisted.
...
| `:hash_already_used_by_claimer` | `claimerUsedHash(claimer, hash)` returned non-zero at `eth_observation_block` (USDC already claimed on Ethereum before the operator signed the mint). |
| `:eth_observation_block_missing` | Row has no positive `eth_observation_block` (legacy or malformed mint). |
| `:eth_observation_block_before_receipt` | Observation block precedes the Locked event's receipt block. |
