...
The unlock authority is gone. There is no `unlock()` callable by the operator. Funds can only move via `claim(preimage)` — which works only if `sha256(preimage) = hash` and only before the deadline — or via `refund(hash)`, which returns funds to the original sender after the deadline passes with no claim. Preimages originate in user wallets: the user picks the preimage, publishes `hash = sha256(preimage)`, and reveals the preimage only at claim time. A compromised 2D operator key cannot forge a 2D HTLC for an attacker — `bridge_lock` only mints when the verifier finds a matching `Locked` event on Ethereum (same hash, same `receiverOn2D`, not yet refunded, not yet consumed by the claimer). Without an Ethereum-side lock under attacker-chosen parameters, no 2D supply can be created.
...
| Threat | Outcome |
|---|---|
| Operator key compromise | Cannot forge a backed mint: every `bridge_lock` requires a matching finalized Ethereum `Locked` event under the same hash, the same `receiverOn2D`, and an unconsumed claimer. Cannot bypass HTLC claim/refund (preimage-locked). Cannot substitute an attacker-controlled HTLC hash (verifier binds `htlc_hash` to Ethereum `topics[1]`). Cannot route funds to an attacker address (verifier binds receiver to `log.data[0:32]`). Cannot replay an already-claimed lock (verifier calls `claimerUsedHash`). Cannot mint against a refunded lock (verifier checks for `Refunded` events). Cannot make plain transfers (block executor blocks non-precompile transfers from the operator address). Operator balance on 2D is always zero: `bridge_lock` atomically mints and locks. Worst case is DoS: the operator stops processing, in-flight HTLCs `refund` after deadline. |
