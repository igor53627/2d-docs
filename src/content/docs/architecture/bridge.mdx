...
Production deploys [`OperatorVault.sol`](https://github.com/igor53627/2d-solidity/blob/60bdbdc38a43f5b8f5eab5f434b6e30c06f85490/src/OperatorVault.sol) as the Ethereum-side operator wallet. The vault is a UUPS-upgradeable smart contract that splits the operator into two roles enforced on-chain.
...
**Signing key (operational).** The signing key has no governance authority and only authorizes `bridgeOut(destination, amount)`, which transfers vault USDC out for bridge-out payouts. It is bounded by three on-chain caps: `destination` must be in `bridgeOutAllowlist`; `amount` must be at most `perTxCap`; and the exact rolling 24h cumulative outflow tracked by timestamped checkpoints must stay at or below `cumulativeCap` after this call.
