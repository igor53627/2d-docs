...
| Threat | Outcome |
|---|---|
| Operator key compromise | Cannot forge a backed refill (verifier catches fabricated events). Cannot bypass HTLC claim/refund (preimage-locked). Cannot touch USD-stable already locked in HTLC swaps. Cannot make plain transfers: the block executor blocks non-precompile transfers from the operator address, so the operator can only move funds through registered precompile calls. **Can** call `refill_mint` against real pending Ethereum locks and immediately lock them via `bridge_lock`; with `bridge_lock`, the funds flow directly into HTLC escrow for the intended recipient, leaving no unbound float. The remaining exposure is limited to mintable-but-unminted Ethereum locks. HTLC eliminates the `unlock()` class of attacks; the operator transfer restriction further reduces custodial risk by preventing direct balance extraction. |
...
The bridge eliminates the `unlock()` authority that has drained wrapped bridges, and binds every mint to a verifier-checked Ethereum event. The operator transfer restriction blocks plain transfers from the operator address, so even a compromised key cannot extract funds except through precompile calls. With `bridge_lock`, refill and HTLC lock happen atomically: there is no window where minted funds sit as unbound operator balance. The remaining exposure is limited to Ethereum locks that have finalized but haven't been processed yet. Funds already locked in HTLC swaps are safe — only `claim(preimage)` or `refund(hash)` can move them.
