...
The unlock authority is gone. There is no `unlock()` callable by the operator. Funds can only move via `claim(preimage)` — which works only if `sha256(preimage) = hash` and only before the deadline — or via `refund(hash)`, which returns funds to the original sender after the deadline passes with no claim. Preimages originate in user wallets: the user picks the preimage, publishes `hash = sha256(preimage)`, and reveals the preimage only at claim time. The verifier's four cross-chain bindings (described below) prevent a compromised 2D operator key from re-routing legitimate user locks to an attacker. They do not, today, bind `claimer` to a configured operator address — see the [trust model](#trust-model-summary) for the residual exposure that creates and the in-progress fix.
...
After confirming the Locked event exists and is finalized, the verifier performs four binding checks:
...
3. **Refund check.** The verifier queries `eth_getLogs` for `Refunded` events matching the HTLC hash and sender within a bounded block window after the lock. If the Ethereum lock was refunded before the operator submitted `bridge_lock`, the mint is rejected.
4. **Single-use claimer hash.** An `eth_call` to `claimerUsedHash(claimer, htlc_hash)` on the bridge contract must return zero, queried at the finalized head as of verification time (not at the receipt block, since `claimerUsedHash` is set only by `claim()` and is therefore always zero at the block where the `Locked` event was emitted). Without this, an operator could re-cite a `Locked` event whose preimage has already been revealed by a prior claim — minting fresh 2D supply against an Ethereum lock that no longer holds USDC.
...
3. **Operator locks on 2D (atomic).** Operator submits `bridge_lock(chain_id, tx_hash, log_index, amount, hash, Alice, deadline)` to `0x2D00…0003`. The precompile atomically inserts an HTLC swap (locking `amount` for Alice under `hash`) and a `bridge_mints` row (supply backing). Operator balance stays at zero throughout. The verifier independently re-checks the Ethereum event and verifies four bindings — `htlc_hash`, `receiverOn2D`, refund status, and that the claimer has not already consumed the hash; on success the block is committed, on failure the block is rejected.
...
| Threat | Outcome |
|---|---|
| 2D-side operator key compromise | An attacker holding only the 2D-side key plus their own USDC capital can mint **unbacked 2D supply**. `BridgeHTLC.lock()` is permissionless on Ethereum, so the attacker self-funds an Ethereum lock with attacker-chosen `(hash, claimer, receiverOn2D)`, cites it via `bridge_lock`, and all four current verifier bindings pass on the self-consistent attacker-controlled lock; the attacker then claims the minted 2D supply with their own preimage and `claim`s or `refund`s the original USDC back on Ethereum. The damage ceiling is the attacker's own working capital, recyclable across hashes. The block executor still blocks plain transfers from the 2D operator address; user-locked USDC under user-chosen preimages is untouched; the operator's accumulated USDC pool is untouched. **Protocol-level fix in progress**: a fifth verifier binding `claimer ∈ operator-allowlist` closes this path; until it ships, the **operational interim** is the 2D-side signing service additionally validating `claimer == configured operator address` in the cited event before signing `bridge_lock`. |
...
The bridge eliminates the `unlock()` authority that has drained wrapped bridges. Every mint is atomically bound to a receiver-specific HTLC via `bridge_lock`, so the operator never holds unbound balance. The verifier independently confirms four bindings (HTLC hash, receiver, refund status, claimer-hash freshness) against finalized Ethereum state before accepting any block containing a mint. Funds locked in HTLC swaps are safe: only `claim(preimage)` or `refund(hash)` can move them.
