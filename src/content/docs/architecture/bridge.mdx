...
5. **Claimer/hash storage probe (TASK-61).** The verifier calls `claimerUsedHash(claimer, htlc_hash)` at `eth_observation_block` persisted on the `bridge_mints` row (consensus state, included in the state root) when the row value is `> 0`. The operator embeds the current Ethereum `"finalized"` block when signing `bridge_lock` (Executor / IntentWatcher enrichment on the honest path); replay uses that frozen value, so a later on-chain claim does not change what historical verifiers read at the observation block. The verifier also requires `receipt_block < eth_observation_block <= finalized_block` at verify time (`observation == receipt_block` is rejected as vacuous). Rows with `eth_observation_block == 0` are pre-TASK-61 migration backfill and skip this probe so historical replay can proceed. Querying `receipt_block` is vacuous (the slot is always zero there); querying the live finalized head for the storage probe breaks replay after a legitimate claim. **TASK-130 (signing path):** `Plan.enrich_eth_observation_block/2` requires the observation to equal Ethereum `"finalized"` at Executor sign time (omitted field is filled; minimized values reject). **Residual:** an operator signing raw calldata while bypassing the Executor still can embed a minimized observation; consensus only bounds `receipt_block < observation <= verify-time finalized`. Closing that bypass needs HSM/OPA binding or attestation (see decision doc in the 2d repo).
...
| `:eth_observation_block_missing` | Row has invalid / non-positive `eth_observation_block` (not the `0` legacy sentinel). |
| `:eth_observation_block_at_receipt` | Observation equals `receipt_block` (vacuous `claimerUsedHash` bypass). |
| `:eth_observation_block_before_receipt` | Observation precedes the Locked event's receipt block. |
| `:eth_observation_block_after_finalized` | Observation exceeds the Ethereum finalized head at verify time. |
...
## Upgrading existing chains (TASK-61 hard fork)
