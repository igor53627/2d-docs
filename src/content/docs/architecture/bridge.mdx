...
**Bridge-out (2D → Ethereum) is not yet live in production.** The current deployment exposes no user-facing bridge-out path; the `OperatorVault.bridgeOut(...)` outflow path described below in the operator policy is reserved for governance-authorized outbound operations. The intended bridge-out design is symmetric to bridge-in — the same role for the operator and the same preimage-driven settlement on both sides, with the operator's accumulated USDC funding bridge-out payouts; if the operator runs out of USDC on Ethereum, bridge-out exits queue until inflows resume — there is no DoS vector that converts to a drain, exits are delayed, never lost. The exact contract surface (vault as `lock` sender, an LP-mediated atomic-swap market, or another shape) is open and tracked separately from this page.
...
**Signing key (operational).** The signing key has no governance authority and only authorizes `bridgeOut(destination, amount)`, which transfers vault USDC out to allowlisted destinations under caps. In the current deployment this path is used only for governance-authorized outbound operations such as treasury rebalances to a multisig — no user-facing bridge-out flow is wired to it (see the [walkthrough](#bridge-in--bridge-out-walkthrough) note). The call is bounded by three on-chain caps: `destination` must be in `bridgeOutAllowlist`; `amount` must be at most `perTxCap`; and the exact rolling 24h cumulative outflow tracked by timestamped checkpoints must stay at or below `cumulativeCap` after this call.
